...
from parallel_Jumping_k import *
...
class Train(object):
    ...
    def build_train_validation_graph(self):
        ...
        logits = inference(self.image_placeholder, FLAGS.num_residual_blocks, reuse=False, is_training=1)
        vali_logits = inference(self.vali_image_placeholder, FLAGS.num_residual_blocks, reuse=True, is_training=0)
    ...
    def test(self, test_image_array):
        ...
        logits = inference(self.test_image_placeholder, FLAGS.num_residual_blocks, reuse=False, is_training=0)
        predictions = tf.nn.softmax(logits)
        ...
        if remain_images != 0:
            self.test_image_placeholder = tf.placeholder(dtype=tf.float32, shape=[remain_images,
                                                        IMG_HEIGHT, IMG_WIDTH, IMG_DEPTH])
            # Build the test graph
            logits = inference(self.test_image_placeholder, FLAGS.num_residual_blocks, reuse=True, is_training=0)
            predictions = tf.nn.softmax(logits)
    ...
    def train_operation(self, global_step, total_loss, top1_error):
        ...
        #REPLACE MY OWN HERE, just for one step
        opt = tf.train.MomentumOptimizer(learning_rate=self.lr_placeholder, momentum=0.9)
        train_op = opt.minimize(total_loss, global_step=global_step,colocate_gradients_with_ops=True)
