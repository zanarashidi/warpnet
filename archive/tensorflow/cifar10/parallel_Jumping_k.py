...
def block_jump4(input,output_channel):
    ...
    with tf_variable_scope('F4') as F4_scope:
...
def inference(input_tensor_batch, n, reuse, is_training):
    ...
    for i in range(n):
        ...
        with tf.variable_scope('conv1_%d' %i, reuse=reuse):
            ...
            out = block_jump(layers[-1],16*k, sr)
            activation_summary(out)
            layers.append(out)
    ...
    for i in range(n):
        with tf.variable_scope('conv2_%d' %i, reuse=reuse):
            ...
            conv2 = block_jump(layers[-1],32*k, sr)
    ...
    for i in range(n):
        with tf.variable_scope('conv3_%d' %i, reuse=reuse):
            ...
            conv3 = block_jump(layers[-1],64*k, sr)
            layers.append(conv3)
